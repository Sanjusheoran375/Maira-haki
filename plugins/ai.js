const {
  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        return await message.send(`*qᴜᴇᴇɴ_ᴍᴀʀɪᴀ ʙᴀʀᴅ ᴀɪ:*\n ${result}`, { quoted: message });
      } catch (error) {
  ...
  smd({
    ...
    try {
      ...
      const { result } = res;
      const astro = "qᴜᴇᴇɴ_ᴍᴀʀɪᴀ ɢᴇᴍɪɴɪ ᴀɪ"
      const tbl = "```";
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("qᴜᴇᴇɴ_ᴍᴀʀɪᴀ• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("qᴜᴇᴇɴ_ᴍᴀʀɪᴀ • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            ...
            try {
              if (_0x2ecd2a) {
                ...
                if (_0x13a11c.toLowerCase().startsWith("reply")) {
                  ...
                  let _0x3f6b59 =
                    "*qᴜᴇᴇɴ_ᴍᴀʀɪᴀ • ʏᴏᴜʀ ᴀɴᴏɴʏ-ᴍsɢ ʀᴇᴘʟʏ*\n\n*_From @" +
  ...
  smd(
    {
      ...
      use: "26371xxxxxxx,Your_text_here",
      fromMe: true,
      filename: __filename,
    },
        ...
        if (!_0x14810d) {
          return await _0x165087.reply(
            "*Advertise of your Message*\n*by sending it to provided nmbr range.*\n" +
              prefix +
              "advt 26371xxxxxxx,Your_text_here"
          );
        }
        ...
        if (!_0xd9b857.includes("x")) {
          return _0x165087.send(
            "*You did not add x in number.*\n*Ex: " +
              prefix +
              "advt 26371xxxxxxx,Your_Message_here*  \n " +
              Config.caption
          );
        }
